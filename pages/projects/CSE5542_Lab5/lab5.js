...
function create_klein_bottle(xSteps, ySteps, color){
    ...
    for(var i = 0; i <= xSteps; i++){
        for(var j = 0; j <= ySteps; j++){
            ...
            if(g.vertex_start_index + ind3 >= vbo_vertices.length / 3){
                console.log("hi")
            }
            ind3 = g.vertex_start_index + ind3 >= vbo_vertices.length / 3 ? 0 : ind3;
...
function create_sphere(xSteps, ySteps, color){ 
    ...
    for(var i = 0; i <= xSteps; i++){
        for(var j = 0; j <= ySteps; j++){
            ...
            var ind2 = ((i+1)*xSteps)+j;
            var ind3 = j == ySteps - 1 ? ((i+1)*xSteps) : ((i+1)*xSteps)+j+1;
            ind3 = ind3+g.vertex_start_index >= vbo_vertices.length / 3 ? 0 : ind3;
            var ind4 = j == ySteps - 1 ? (i*xSteps) : (i*xSteps)+j+1;
            g.vertex_element_index.push(g.vertex_start_index + ind1);
            g.vertex_element_index.push(g.vertex_start_index + ind2);
            g.vertex_element_index.push(g.vertex_start_index + ind3);
...
function loadOBJfile(location, translate, rotate, scale, color, useTexture=null, useCubemap=null, useNormalMap=null){
...
function initScene() {
    ...
    goodBrick2.localScale = [50, 50, 50];
    goodBrick2.translate = [-30, 30, 50];
    ...
    head.root = chest;
    head.localScale = [5.4 * (1/head.root.global_scale()[0]), 5 * (1/head.root.global_scale()[1]), 5.4 * (1/head.root.global_scale()[2])];
    ...
    rightHand = create_sphere(10, 10, [0.6, 0.1, 0.1]);
    rightHand.name = "Right hand";
    rightForearm.children.push(rightHand);
    rightHand.root = rightForearm;
    rightHand.localScale = [3*(1/rightHand.root.global_scale()[0]), 3*(1/rightHand.root.global_scale()[1]), 3*(1/rightHand.root.global_scale()[2])];
    ...
    leftHand = create_sphere(10, 10, [0.6, 0.1, 0.1]);
    leftHand.name = "Left hand";
    leftForearm.children.push(leftHand);
    leftHand.root = leftForearm;
    leftHand.localScale = [3*(1/leftHand.root.global_scale()[0]), 3*(1/leftHand.root.global_scale()[1]), 3*(1/leftHand.root.global_scale()[2])];
...
function update(){
    ...
    var goodBrick2 = findGameobject("goodBrick2");
    ...
    goodBrick2.textureOffset = [goodBrick2.textureOffset[0]+0.01, goodBrick2.textureOffset[1]+0.015];
    CreateBuffers();
    draw_scene();
}
/**
...
*/
function onDocumentMouseMove( event ) {var mouseX = event.clientX;
    ...
    if(enableMouseMove){    
        ...
        var dx = NDC_X - start_NDC_X;
        var dy = NDC_Y - start_NDC_Y;

        cam_pitch_yaw_roll[0] = cam_pitch_yaw_roll[0] + 90*-dy;
        cam_pitch_yaw_roll[1] = cam_pitch_yaw_roll[1] + 90*dx;
...
function get_eye_vectors(){
    ...
    right = vec3.normalize(vec3.create([-rot[0], -rot[4], -rot[8]]));
    up = vec3.normalize(vec3.create([rot[1], rot[5], rot[9]]));
    forward = vec3.normalize(vec3.create([rot[2], rot[6], rot[10]]));
